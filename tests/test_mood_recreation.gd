...
func _test_hud_contract() -> void:
	...
	_assert_true(checklist_scroll != null and checklist_scroll.custom_minimum_size.y >= 180.0, "briefing checklist scrolls above a sticky action footer")
	_assert_true(orders.checklist.fit_content, "briefing checklist still expands for objective text inside the scroll")
	...
	_assert_true(orders.briefing_begin_button.get_global_rect().end.y <= orders.briefing_panel.get_global_rect().end.y + 1.0, "BEGIN SHIFT stays inside the briefing panel")
	_assert_true(orders.briefing_tips_label != null and "HELP" in orders.briefing_tips_label.text.to_upper(), "opening tips stay one short line")
