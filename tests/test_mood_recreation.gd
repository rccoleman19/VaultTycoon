...
func _test_hud_contract() -> void:
	...
	orders.refresh()
	_assert_equal(orders.command_grid.columns, 7, "expanded command grid retains two-row layout")
	_assert_equal(orders.command_grid.get_child_count(), 13, "toolbar contains eleven tools plus save and load")
	...
	_assert_true("[    ][/color]  Power a rec console" in orders.checklist.text, "an unfinished console does not complete the powered-console objective")
	...
	orders.refresh()
	_assert_true("[DONE][/color]  Power a rec console" in orders.checklist.text, "a powered completed console satisfies its briefing objective")
